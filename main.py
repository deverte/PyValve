...
"""
TODO:
1. Добавить проверку доступа к http-серверу.
2. В плане, возможно стоит убрать Priority и ориентироваться только на Pressure,
или добавить колонку, которая определяет верхняя, или нижняя граница задается.
Так же, добавить возможность ставить несколько каналов (например 1:3:5 -
проверка по первому, третьему и пятому каналу, +:-:+ - по первому каналу верхняя
граница, по третьему - нижняя, по пятому - верхняя, 100:500:800 - по первому
каналу 100 мм рт.ст., по третьему - 500 мм рт.ст., по пятому - 800 мм рт.ст.).
...
class Supervisor:
    ...
    def sequence(self):
        ...
        if self.stage < len(plan):
            ...
            # Завершить цикл
            self.is_end = True
            # (!) Возможно, удобнее будет добавить флаг выключения по окончанию
            # в файл настроек
            # Закрытие всех реле
            off = settings.loc["off"][0]
            # <DEBUG>
            print("/".join([settings.loc["ip"][0], client.command("1" * 10, off)]))
            # <RELEASE>
            #send(settings.loc["ip"][0], client.command("1" * 10, off))
